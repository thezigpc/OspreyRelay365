...
public class SetupWizardForm : Form
{
    ...
    private AppRegistrationManager BuildAdminManager()
    {
        ...
        var graphClient = OspreyRelay.M365.Graph.GraphClientFactory.CreateWithToken(_adminAccessToken);
    ...
    private async Task RunTestAsync()
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(cfg.FallbackSenderEmail))
            {
                try
                {
                    var client = OspreyRelay.M365.Graph.GraphClientFactory.Create(cfg);

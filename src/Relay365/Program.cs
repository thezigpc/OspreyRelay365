...
using Relay365.Service;

// Elevated reinstall/uninstall triggered by the UI via runas
if (args.Contains("--installservice"))
{
    Relay365.Services.WindowsServiceManager.Install(Environment.ProcessPath!);
    return;
}
if (args.Contains("--uninstallservice"))
{
    Relay365.Services.WindowsServiceManager.Uninstall();
    return;
}
...
if (!isFirstInstance)
{
    var hwnd = Relay365.NativeMethods.FindWindow(null, "365 Email Relay");
    if (hwnd != IntPtr.Zero)
    {
        Relay365.NativeMethods.ShowWindow(hwnd, Relay365.NativeMethods.SW_RESTORE);
        Relay365.NativeMethods.SetForegroundWindow(hwnd);
    }
    return;
}

ApplicationConfiguration.Initialize();
Application.Run(new Relay365.Forms.MainForm());
